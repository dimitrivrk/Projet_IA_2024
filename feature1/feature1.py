...
from feature1_anomaly_detection import anomaly_detection_OneClassSVM, anomaly_detection_IsolationForest, plot_anomalies
...
def evaluate_clusters(df_learning, max_clusters) -> 'plots':
    """
    This function evaluates the clusters by plotting the silhouette scores, inertia and explained variance
    :param df_learning: the data frame with the selected columns for learning
...
def plot_cluster(df, k=3) -> 'plot map':
    """
    This function plots the trees on a map with different colors for each cluster
...
def find_best_k() -> 'plots':
    """
    Use the silhouette score and the inertia to find the best number of clusters
    With the inertia method there is no clear elbow, and with the silhouette score the best k is 2 but the curve is monotonic
    :return: check the plots
    """
    df, df_learning = load_data()
    evaluate_clusters(df_learning, 10)


def show_map(k=3) -> 'map in html file':
    """
    Use folium/leaflet to display a map with the trees colored by cluster
    if k <= 3 click on a tree to see if it is small, medium or big
    :return:
    """
    df, df_learning = load_data()
    df = cluster(df, df_learning, k)
...
def detect_anomalies():
    ...
    anomalies_indexes = anomaly_detection_OneClassSVM(df_learning)
    print(f'OneClassSVM detected {len(anomalies_indexes[0])} anomalies')

    anomalies_indexes_ = anomaly_detection_IsolationForest(df_learning, bootstrap=True)
...
def plot_number_anomalies(ocsvm: bool = True, iforest: bool = True):
    ...
    if iforest:
        contamination_values = np.arange(0.01, 1, 0.01)
